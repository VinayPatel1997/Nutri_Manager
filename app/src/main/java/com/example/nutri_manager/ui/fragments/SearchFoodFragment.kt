...
class SearchFoodFragment : Fragment(R.layout.fragment_search_food) {
            ...
            job = MainScope().launch {
                ...
                editable?.let {
                    if (editable.toString().isNotEmpty()) {
                        viewModel.searchFoodsPage = 1
                        viewModel.searchFoodsResponse = null
                        viewModel.searchFoods(editable.toString())
                    }
                }
            }
        }

        viewModel.searchFoodsMutableLiveData.observe(viewLifecycleOwner, Observer { response ->
            when (response) {
                is Resource.Success -> {
                    hideProgressBar()
                    response.data?.let { foodsResponse ->
                        Toast.makeText(context, "New Record Fetched!", Toast.LENGTH_SHORT).show()
    ...
    val scrollListener = object : RecyclerView.OnScrollListener() {
        override fun onScrolled(recyclerView: RecyclerView, dx: Int, dy: Int) {
            ...
            if(shouldPaginate) {
                viewModel.searchFoods(etSearch.text.toString())
                isScrolling = false
            }
        }
